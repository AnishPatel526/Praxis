...
router.post("/", (req: Request, res: Response) => {
  ...
  void (async () => {
    try {
      ...
      const action_id  = action.action_id.trim();
      const incidentId = action.value.trim();
      console.log(`[ChatOps] raw action_id="${action.action_id}" cleaned="${action_id}" | raw value="${action.value}" cleaned="${incidentId}"`);

      const originalBlocks = payload.message?.blocks ?? [];
      const responseUrl = payload.response_url;

      // Guard against SSRF: only post back to Slack's own hook domain
      const isSafeResponseUrl =
        !responseUrl || responseUrl.startsWith("https://hooks.slack.com/");
      if (responseUrl && !isSafeResponseUrl) {
        console.error(`[ChatOps] Untrusted response_url rejected: ${responseUrl}`);
        return;
      }
